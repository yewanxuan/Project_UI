            ...
            <span class="wd-nav-choise">
                <c:forEach var="item" items="${sessionScope.typeMap}" end="5">
                    <li class="layui-nav-item">
                    	<%-- <a href="info_ListShow.action?infoType=${item.key}">${item.value}</a> --%>
                    	<a href="info_SearchShow.action?subsql=info_typepid&sqlvalue=${item.key}&type=all">${item.value}</a>
                    </li>
                </c:forEach>
                <li class="layui-nav-item">
                    <a href="javascript:;">></a>
                    <dl class="layui-nav-child">
                        <c:forEach var="it" items="${sessionScope.typeMap}" begin="6" end="10">
                            <dd><a href="info_SearchShow.action?subsql=info_typepid&sqlvalue=${it.key}&type=all">${it.value}</a></dd>
                        </c:forEach>
                    </dl>
                </li>
            </span>
