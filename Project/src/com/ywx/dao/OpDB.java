...
public class OpDB {
		...
		try{
			...
			if(rs!=null){
				while(rs.next()){
					...
					System.out.println(infoSingle.getInfoTitle()+infoSingle.getInfoContent());
		...
		try{
			...
			if(rs!=null){
				while(rs.next()){
					...
					comment.setReqDate(DoString.dateTimeChange(rs.getTimestamp("req_date")));
					comment.setReqName(rs.getString("req_name"));
					comment.setRspName(rs.getString("rsp_name"));
					System.out.println("CommentSingle is"+comment.getId()+comment.getReqUserid()+comment.getRspUserid()+comment.getDetail()+comment.getReqDate()+comment.getInfoId()+comment.getReqName()+comment.getRspName());
	...
	public TreeMap OpGetNameBox(String sql,Object []params){
		TreeMap typeMap = new TreeMap();
		mydb.doPstm(sql, params);
		...
		try {
			ResultSet rs = mydb.getRs();
			while(rs.next()){
				typeMap.put(rs.getInt("id"), rs.getString("name"));
			}
			rs.close();
		} catch (SQLException e) {
			e.printStackTrace();
